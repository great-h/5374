...
var TrashModel = function(_lable, _cell, remarks) {
  ...
  this.getDateLabel = function() {
    ...
    return this.mostRecent.getFullYear() + "/" + (1 + this.mostRecent.getMonth()) + "/" + this.mostRecent.getDate() + ' (' + day_enum[this.mostRecent.getDay()] + ')';
  }
    ...
    for (var i in areaModel.trash) {
	  ...
	  if (trash.mostRecent === undefined) {
	    leftDayText == "不明";
          ...
          accordionHTML += "</div>" +
            '<h6><p class="text-left remark">' + remark + "</p></h6>" +
            "</a>" +
            "</div>" +
